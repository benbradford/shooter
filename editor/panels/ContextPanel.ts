...
export class ContextPanel {
  ...
  async showStatePanel(): Promise<void> {
    ...
    this.container.querySelector('#st-reset-flags')?.addEventListener('click', () => {
      const resetFlags: Record<string, string> = { canPunch: 'true', canSwim: 'true', canJump: 'true', canPush: 'true', hasSuperPunch: 'true', hasAutoHeal: 'false', hasCompanion: 'false', pet_rock_collected: 'true', pet_dog_collected: 'true', pet_selected: 'rock' };
      const flagsDiv = this.container.querySelector('#st-flags')!;
      flagsDiv.innerHTML = '';
  ...
  showLevelInfo(): void {
    ...
    this.container.innerHTML = `
      ...
      <div class="level-info-grid">
        ...
        <span class="label">Music</span><select id="li-music" style="font-size:11px">${['(none)', 'btr_music', 'btr_overworld', 'btr_wilds', 'btr_tonal'].map(m => `<option value="${m === '(none)' ? '' : m}" ${(levelData.music ?? '') === (m === '(none)' ? '' : m) ? 'selected' : ''}>${m}</option>`).join('')}</select>
