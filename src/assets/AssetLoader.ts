...
export function getRequiredAssetGroups(levelData: LevelData): AssetGroupKey[] {
  ...
  if (levelData.entities) {
    ...
    if (entityTypes.has('skeleton') || entityTypes.has('red_skeleton')) {
    ...
    if (entityTypes.has('bell')) {
      groups.push('bell');
    }
    if (entityTypes.has('worm')) {
      groups.push('worm');
    }
  ...
  if (!groups.includes('escort')) {
    ...
    if (ep.getCurrentEscortId() || ep.getCompletedEscortIds().length > 0) {
      groups.push('escort');
    }
  }
