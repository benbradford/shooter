...
interface LevelData {
  ...
  levelTheme?: 'dungeon' | 'swamp' | 'grass';
  background?: LevelBackground;
}

interface LevelEntity {
  id: string;                    // Unique ID (e.g., "skeleton0", "thrower1")
  type: EntityType;              // Entity type
  createOnEvent?: string;        // Optional - entity spawns when this event fires
  data: Record<string, unknown>; // Entity-specific data (col, row, difficulty, etc.)
}
...
All entities (enemies, triggers, exits, event chainers) are defined in a single `entities` array. Each entity has:

- **id**: Unique identifier, auto-generated by editor (e.g., "skeleton0", "robot1")
- **type**: Entity type (skeleton, thrower, stalking_robot, bug_base, bullet_dude, trigger, exit, eventchainer)
- **createOnEvent**: Optional - if set, entity only spawns when this event fires
- **data**: Entity-specific properties (col, row, difficulty, waypoints, etc.)

### Immediate vs Event-Driven Creation

**Immediate** (spawns on level load):
```json
{
  "id": "skeleton0",
  "type": "skeleton",
...
**Event-driven** (spawns when event fires):
```json
{
  "id": "skeleton1",
  "type": "skeleton",
...
Triggers fire events when the player enters their cells:
...
EventChainers raise multiple events sequentially with delays (replaces old spawner system):
...
2. trigger1 fires "spawn_wave" event
3. eventchainer1 (createOnEvent: "spawn_wave") spawns and starts
4. eventchainer1 raises "sk1" immediately (0ms delay)
5. skeleton1 (createOnEvent: "sk1") spawns
6. eventchainer1 raises "sk2" after 1000ms
7. skeleton2 (createOnEvent: "sk2") spawns
8. eventchainer1 destroys itself
...
- **Add** - Opens entity type dropdown to place new entities
...
Unified entity placement with dropdown selection:

**Usage:**
1. Click **Add** button
2. Select entity type from dropdown (Skeleton, Thrower, Robot, Bug Base, Bullet Dude, Trigger, Exit, Event Chainer)
3. Ghost sprite appears following mouse
4. Click to place entity
5. Entity ID auto-generated (lowest available number)
6. Stays in add mode for placing multiple entities
...
**Important:**
- Triggers are entities with type "trigger"
- `oneShot` defaults to true to prevent duplicate events
- Events are raised through `EventManagerSystem.raiseEvent(eventToRaise)`

### Portal Mode

Allows creating level exits/transitions - portals that transport the player to another level.

**Note:** Exits are now part of the unified entity system.
...
- `DefaultEditorState` - Main menu with buttons (two rows), shows entity ID on click
- `AddEntityEditorState` - Unified entity placement with dropdown selector
...
The editor auto-generates unique IDs when placing entities:
- Format: `{type}{number}` (e.g., skeleton0, skeleton1, thrower0)
- Uses lowest available number starting from 0
- Checks both level data and currently spawned entities for uniqueness
