...
- **Regression test infrastructure (2026-05-20)**: New `test/interactions/` helper modules: `combat.js` (punch state, enemy health/count, waitForPunchComplete), `flags.js` (WorldState flag get/set/wait, CachedFlag sync), `state.js` (player state machine, health, death, water detection), `hud.js` (joystick visuals), `input.js` (low-level joystick touch simulation). New test levels: `test-combat.json`, `test-punch-damage.json`. New globals exposed in test mode: `WorldStateManager`, `AttackComboComponent`, `HealthComponent`, `WalkComponent`, `StateMachineComponent`, `WaterEffectComponent`. New test files: `test-combat.js` (canPunch flag gating), `test-punch-damage.js` (punch deals damage to enemies), `test-cached-flag-load.js` (CachedFlag loadFromFile regression). Feature spec in `features/regression-tests/`. Motivation: CachedFlag migration caused a punch regression — tests now catch flag-gating bugs.
