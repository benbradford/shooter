...
- **currentCell uses center of collision box**: `GridCollisionComponent` now computes `currentCell` from the center of the entity's collision box (not top-left corner). Fixes exits/triggers not firing when collision box offset pushed the top-left into an adjacent cell.
...
- **Loading screen image**: BootScene now shows `Loading.png` image (full-screen, display-sized) instead of text during initial asset loading. Minimum 1000ms display time before transitioning to title.
