...
- `creating-chatgpt-image-prompts.md` — How to write effective image-gen prompts for one-off props, terrain tiles, and full tileset families (trigger phrases: "help me create a chatgpt prompt", "image prompt for", "help me design a tileset", etc.)
...
- **ChatGPT tileset-planning SOP update (2026-07-17)**: `agent-sops/creating-chatgpt-image-prompts.md` now distinguishes one-off props, single terrain tiles, and full tileset-family requests. Added a planning-first workflow for new level themes: required tileset questions, material-vs-topology guidance, minimum viable asset set, generation order (fills before transitions before props), and updated output rules so agents return a tileset plan plus staged prompt batches instead of several unrelated prompts.
...
- **Worm enemy (2026-05-29)**: New `worm` entity type. 4-directional enemy that wanders slowly (40px/s) and spits projectiles at the player when within 350px (4.5s cooldown). Spit projectile: 200px/s, 5 damage, max 5 cells. Sounds: `worm_spit` on spit. Key files: `src/ecs/entities/worm/WormEntity.ts`, `WormWanderState.ts`, `WormSpitState.ts`, `WormSpitProjectileEntity.ts`, `WormAnimations.ts`. Registered in `enemyFactories.ts`.
- **Beetle enemy (2026-05-30)**: New `beetle` entity type. 4-directional enemy that wanders, detects player (250px), charges in a cardinal direction (250px/s, up to 5 cells, 5 damage), and leaves a toxic puddle on death (4s duration, 5 DPS, 30px radius). Sounds: `beetle_splat` on death. Health drop: 15%. Key files: `src/ecs/entities/beetle/BeetleEntity.ts`, `BeetleWanderState.ts`, `BeetleChargeState.ts`, `BeetleDeathState.ts`, `ToxicPuddleEntity.ts`, `BeetleAnimations.ts`. Registered in `enemyFactories.ts`.
- **Exit preserveCol/preserveRow (2026-05-29)**: New optional fields on exit entities. When `preserveCol: true`, player spawns at their current column instead of `targetCol`. When `preserveRow: true`, player spawns at their current row instead of `targetRow`. Useful for multi-cell exits spanning a level edge. Implementation in `LevelExitComponent.ts` reads player's `GridPositionComponent.currentCell`.
- **Water exits (2026-05-29)**: If the target spawn cell is a water cell (and not a bridge), the player spawns already swimming — no jump-in animation plays. Uses `WaterEffectComponent.enterWaterImmediate()` (new method) to set swimming state, push swimming collision box, and set swim depth. `GameScene` checks spawn cell properties after player creation.
- **New enemy spritesheets (2026-06-01)**: Generated spritesheets for 4 upcoming enemy types: `mole`, `eye`, `frog`, `fly`. Assets in `public/assets/{name}/` with `{name}_spritesheet.png`, `frame_list.txt`, `metadata.json`, and `rotations/` directory. Registered in `AssetRegistry.ts` with asset groups. No entity type implementations yet — assets only.
- **Swimming detection fix (2026-06-02)**: Fixed bug where swimming detection was too lenient when walking south onto water. Collision detection now based on the player's actual collision box position rather than visual center.
- **Pet jump position fix (2026-06-03)**: Fixed pet snapping back to pre-jump position after platform/void jumps. `PetSyncJumpBehavior.finishJump()` now calls `gridCollision.syncPreviousPosition()`. Same fix applied when exiting water ride.
- **Pet void-fall respawn fix (2026-06-03)**: After void falls, pet teleports to player's feet cell instead of jump target. Prevents pet spawning on void cells.
- **Pet void escape safety net (2026-06-03)**: `PetFollowComponent.update()` checks each frame if pet is on void cell — if so, immediately teleports to player's feet cell.
- **Pet layer sync removed (2026-06-03)**: `PetFollowComponent.syncPetLayer()` is now a no-op. Pet's layer determined by `GridCollisionComponent` from actual cell. `PetSyncJumpBehavior.finishJump()` reads landing cell's layer directly.
- **Pet wander blocking (2026-06-03)**: Wander targets on void cells, walls, blocked areas, and cells on a different layer are all rejected by `PetFollowComponent`.
- **Pet tests (2026-06-03)**: New test files in `test/tests/pets/`: `test-pet-fall-respawn.js`, `test-pet-platform-fall.js`, `test-pet-void-escape.js`, `test-pet-wander-blocked.js`, `test-pet-water.js`, `test-water-edge-cases.js`, `test-pet-teleport.js`, `test-rapid-swap.js`, `test-pet-ability-disabled.js`, `test-invalid-state.js`, `test-pathfinding-edge.js`, `test-level-transition-pets.js`.
- **Beetle combat test (2026-06-05)**: New test file `test/tests/combat/test-beetle.js` with test level `public/levels/test/test-beetle.json`.
- **Dynamic Z for background textures (2026-06-05)**: New `dynamicZ` boolean field on background textures. When set, sprite renders in front of or behind player based on Y position. `BackgroundTextureRenderer.updateDynamicZ(playerY)` flips depth each frame. Editor: Dynamic Z checkbox in cell texture form.
- **Jump sound (2026-06-06)**: `jump_hup` sound plays on takeoff in `JumpAnimator.ts`. Registered in `core` asset group (always loaded).
- **Conditional background textures (2026-06-07)**: New `conditionalTextures` field on `LevelCell`. Evaluates a WorldState flag at level load and selects which textures to display. Type: `{ flag, cases: [{value, textures}], default }`. Resolved in `BackgroundTextureRenderer.resolveTextures()`. Editor: ContextPanel shows conditional texture fields with flag name, case values, and defaults. Use case: showing different cell appearance based on quest progression (e.g., door open/closed).
- **Fly enemy (2026-06-07)**: New `fly` entity type. Airborne enemy that bobs randomly near start location (30px radius), detects player (120px), swoops down to attack (15 damage, 150px/s), only vulnerable during swoop (down + overshoot phases). 1 HP. 2s cooldown between swoops. No health drop. Key files: `src/ecs/entities/fly/FlyEntity.ts`, `FlyAnimations.ts`, `src/ecs/components/fly/FlyBehaviorComponent.ts`. Registered in `enemyFactories.ts`.
- **Drawbridge spritesheet (2026-06-07)**: `drawbridge_spritesheet` added to `AssetRegistry`, `editor` asset group, and `SpritesheetTextures.ts` with 3 sub-sprites: `drawbridge_closed`, `drawbridge_mid`, `drawbridge_open`.
- **Bell elliptical shockwaves (2026-06-07)**: Bell shockwave rings now expand elliptically (scaleX: 8, scaleY: 5) for top-down perspective instead of uniform circular expansion.
- **Doc guidance: no constants (2026-06-08)**: Updated documentation principles — don't document specific constants (detect distances, damage values, cooldowns, speeds) as these are trivially found in code and change frequently. Focus on behavioral descriptions, pitfalls, and non-obvious patterns instead. Trimmed all enemy descriptions across docs accordingly.
...
- **Moving tile system (2026-07-31)**: New `moving_tile` entity type. A platform-sized grid occupant that follows a scripted path of wait/move steps and carries riders (player, pet) by applying its frame delta to their transforms. `GridMovementValidator` special-cases boarding (always allowed), riding (bypasses water/layer checks for cells covered by the tile), and leaving (re-applies target cell rules). Script parsed from level JSON via `parseMovingTileScript()`. Key files: `src/ecs/components/moving-tile/MovingTileComponent.ts`, `MovingTileScript.ts`, `src/ecs/entities/moving-tile/MovingTileEntity.ts`. Depth: `Depth.movingTile` (-39). Tests: `test/tests/player/test-moving-tile.js`.
- **Coin spin animation (2026-07-31)**: Coins spawned from breakables now spin in the direction they're emitted (left→counter-clockwise, right→clockwise). Spin speed decays exponentially via `scaleX` squash (`SPIN_DECAY_TIME_CONSTANT_SEC = 0.9`). Visual achieved by modulating `SpriteComponent.visualScaleX` with cosine phase. Tests: `test/tests/pickups/test-coin-spin.js`.
- **Breakable shard rotation decay (2026-07-31)**: Shard fragments from broken breakables and root chests now rotate with exponential decay (slows over time) instead of constant speed. Uses `decayingRotationAngleDeg()` utility from `src/utils/ShardRotation.ts`. Tests: `test/tests/breakables/test-breakable-shard-spin.js`, `test-root-chest-shard-spin.js`.
- **NPC asset folder restructure (2026-07-31)**: NPC rotation assets moved from `public/assets/old_village_lady/` to `public/assets/npc/old_village_lady/`. NPC assets now organized under `public/assets/npc/{name}/` subfolder pattern.
- **Old village lady NPC (2026-07-31)**: New NPC spritesheet `old_village_lady` added. Registered in `AssetRegistry.ts` with asset group. Usable in editor as NPC asset.
- **Moving tile water bug lesson (2026-07-31)**: Moving tile across water caused the player to immediately jump off into the water. Root cause: `GridMovementValidator` treated water cells under a moving tile as normal water and blocked the rider. Fix required adding moving-tile-awareness to `validatePosition()` — checking both `findMovingTileCovering()` and the riding tile's `coversCell()` to bypass water/layer rules for cells the tile geometrically covers. **Lesson learned**: Initial fix attempts failed because the agent guessed at solutions without reproducing the bug in a test first. Only succeeded after writing `test-moving-tile.js` (specifically the water-crossing scenario) and running it to confirm the failure, then fixing the validator logic and confirming the test passed.
