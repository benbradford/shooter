...
- **Worm enemy (2026-05-29)**: New `worm` entity type. 4-directional enemy that wanders slowly (40px/s) and spits projectiles at the player when within 350px (2.5s cooldown). Spit projectile: 200px/s, 5 damage, max 5 cells. Key files: `src/ecs/entities/worm/WormEntity.ts`, `WormWanderState.ts`, `WormSpitState.ts`, `WormSpitProjectileEntity.ts`, `WormAnimations.ts`. Registered in `enemyFactories.ts`.
- **Beetle enemy (2026-05-30)**: New `beetle` entity type. 4-directional enemy that wanders, detects player (250px), charges in a cardinal direction (250px/s, up to 5 cells, 5 damage), and leaves a toxic puddle on death (2s duration, 5 DPS, 30px radius). Key files: `src/ecs/entities/beetle/BeetleEntity.ts`, `BeetleWanderState.ts`, `BeetleChargeState.ts`, `BeetleDeathState.ts`, `ToxicPuddleEntity.ts`, `BeetleAnimations.ts`. Registered in `enemyFactories.ts`.
