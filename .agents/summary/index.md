...
- **Bell entity (2026-05-27)**: New `bell` entity type. Punchable bell that swings with damped oscillation, emits shockwaves, cracks at 60% progress, and raises `{levelName}_{entityId}_rung` event on completion. Persists via WorldState (shows cracked on re-entry). Key files: `src/ecs/entities/bell/BellEntity.ts`, `src/ecs/entities/bell/BellComponent.ts`. Registered in `gameplayFactories.ts`.
