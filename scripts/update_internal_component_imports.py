...
def update_component_imports(file_path):
    ...
    for component, target_folder in COMPONENT_MAP.items():
        if current_folder == target_folder:
            # Same folder: from './ComponentName'
            pattern = rf"from (['\"])\.\./{component}(['\"])"
            replacement = rf"from \1./{component}\2"
            content = re.sub(pattern, replacement, content)
        else:
            # Different folder: from '../folder/ComponentName'
            pattern = rf"from (['\"])\.\./{component}(['\"])"
...
for ts_file in base_dir.rglob('*.ts'):
    if update_component_imports(ts_file):
        updated += 1

print(f"\nTotal component files updated: {updated}")
